...
class ComputersLoader(BaseGraphLoader):
    ...
    def load_data(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.iid:
            ...
            if len(client_nodes) < self.num_clients:
                print(f"Warning: Louvain produced only {len(client_nodes)} clients. Falling back to IID.")
                client_nodes = self._partition_iid(data.num_nodes)
    ...
    def _partition_louvain(self, data) -> Dict[str, torch.Tensor]:
        ...
        sorted_communities = sorted(community_to_nodes.items(), key=lambda x: len(x[1]), reverse=True)
        client_nodes = {}
    ...
    def _create_client_data(self, data, node_ids, client_id, global_label_dist):
